...
export_all_results <- function(all_clusters, final_clusters, cluster_labels,
  ...
  sample_table <- data.frame(
    ...
    cluster_label = paste("Cluster", cluster_labels[as.character(final_clusters)]),
    row.names   = NULL
  )
  ...
  if (!is.null(bio_table)) {
    ...
    df_bio$cluster_id <- as.integer(rownames(df_bio))
  ...
  # Tabla conjunta de los 20 marcadores positivos por cluster
  top_marker_table <- do.call(rbind, lapply(names(deg_results$top_markers), function(cl_name) {
    df <- deg_results$top_markers[[cl_name]]
    if (is.null(df) || nrow(df) == 0) return(NULL)
    data.frame(
      cluster_id = as.integer(cl_name),
      gene = rownames(df),
      log2FoldChange = df$log2FoldChange,
      foldChange = 2^df$log2FoldChange,
      pvalue = df$pvalue,
      FDR = df$padj,
  ...
  if (!is.null(top_marker_table)) {
    save_table(top_marker_table, "top20_up_markers_by_cluster.csv",
  ...
  ann_summary <- data.frame(
    ...
    cluster_label = paste("Cluster", unname(cluster_labels)),
