...
DATA_FILE <- file.path("data", "strap_counts.RData")
META_FILE <- file.path("data", "E-MTAB-13733.sdrf.txt")
...
source(file.path("pipeline", "functions", "00_setup.R"))
source(file.path("pipeline", "functions", "01_preprocessing.R"))
source(file.path("pipeline", "functions", "02_dimreduction.R"))
source(file.path("pipeline", "functions", "03_clustering.R"))
source(file.path("pipeline", "functions", "04_validation.R"))
source(file.path("pipeline", "functions", "05_deg_analysis.R"))
source(file.path("pipeline", "functions", "06_annotation.R"))
source(file.path("pipeline", "functions", "07_visualization.R"))
source(file.path("pipeline", "functions", "08_export.R"))
...
cat("\n=== [6/8] ANOTACIÓN BIOLÓGICA POST HOC ===\n")
...
# La anotación se conserva para la discusión, pero las salidas principales
# utilizan exclusivamente IDs numéricos de cluster.
biological_labels <- annotation_res$labels
cluster_labels <- numeric_labels
cluster_colors <- setNames(
  RColorBrewer::brewer.pal(max(3, length(cluster_labels)), "Set2")[seq_along(cluster_labels)],
  names(cluster_labels)
)
cat("  Anotación:", paste(names(cluster_labels), "→", cluster_labels,
                           collapse = " | "), "\n")

# Los DEGs permanecen identificados como clusters 1..5.
...
cat("  Clusters:", paste(cluster_labels, collapse = " | "), "\n\n")
