...
# 1. Firmas génicas de referencia para AR sinovial

RA_SIGNATURES <- list(

  # 1.1. Fibroblastos sinoviales activados
  
  # Basado en Croft et al. (2019) Nature; Buckley et al. (1999) J Immunol
  
  Fibroid  = c("CXCL12", "PDPN", "PRG4", "THY1", "CDH11", "FAP",
               "ACTA2", "COL1A1", "COL3A1", "FN1"),

  # 1.2. Macrófagos/monocitos proinflamatorios
  
  # Basado en Humby et al. (2019) Ann Rheum Dis; McInnes & Schett (2011) NEJM
  
  Myeloid  = c("CD68", "MRC1", "CSF1R", "IL1B", "TNF", "IL6",
               "ITGAM", "CD14", "FCGR3A", "CCL2"),

  # 1.3. Linfocitos T y B en agregados linfoides sinoviales
  
  # Basado en Takemura et al. (2001) J Immunol; Humby et al. (2009) PLoS Med
  Lymphoid = c("CD3D", "CD3E", "CD19", "MS4A1", "SELL", "PTPRC",
               "CD4", "CD8A", "FOXP3", "CXCR5"),

  # 1.4. Respuesta a interferón tipo I
  
  # Basado en Higgs et al. (2011) Arthritis Rheum; Boyle et al. (2021) ARD
...
# 2. Puntuación de firma para cada muestra
...
# 3. Asignar nombre biológico a cada cluster
...
annotate_clusters <- function(clusters, expr, meta, k,
  ...
  # 3.1. Puntuación de cada muestra en cada firma
  sig_scores <- lapply(RA_SIGNATURES, score_signature, expr_scaled = expr)
  sig_mat    <- do.call(cbind, sig_scores)

  # 3.2.Puntuación media de firma por cada cluster
  ...
  # 3.3: Uso del solapamiento histológico para anclar nombres clásicos
  if ("pathotype_clean" %in% colnames(meta)) {
    ...
    # Para cada cluster, se le asigna el patotipo histológico dominante
  ...
  # 3.4. Construcción de etiquetas combinando la firma génica y el solapamiento histológico
  ...
  if (!is.null(histological_match) && !is.null(ct_pct)) {
    ...
    # Para los clusters sin asignación histológica clara se emplea la firma génica
    ...
    if (labels[cl_char] == "") {
      ...
      if (!candidate %in% used_names) {
        ...
        used_names       <- c(used_names, candidate)
          ...
          # Si el nombre ya está usado, asignamos un nombre genérico numérico
  ...
  # 3.5. Orden de etiquetas según el orden biológico canónico
  
  canonical_order <- c(pathotype_order, "IFN-high", "Endothelial",
                       paste0("Subtype", 1:10))
  ordered_labels  <- labels[order(match(labels, canonical_order,
                                        nomatch = 999))]

  # 3.6 Asignación de colores consistentes en el orden canónico
...
# 4. Reetiqueta un vector de clusters con los nombres biológicos
