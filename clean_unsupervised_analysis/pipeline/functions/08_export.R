...
export_all_results <- function(all_clusters, final_clusters, cluster_labels,
  ...
  best_method <- if (!is.null(validation$selected_method)) {
    validation$selected_method
  } else {
    validation$ranking$method[1]
  }

  # 1. Asignaciones de cluster
  ...
  sample_table <- data.frame(
    ...
    row.names   = NULL
     ...
     # Añadimos el  patotipo histológico si está disponible
  ...
  # Añadir coordenadas PC1 y PC2
  if (!is.null(pca_res) && !is.null(pca_res$coords)) {
    idx_pca <- match(sample_table$sample_id, rownames(pca_res$coords))
    sample_table$PC1 <- pca_res$coords[idx_pca, 1]
    sample_table$PC2 <- pca_res$coords[idx_pca, 2]
  }
  ...
  # 3. DEGs separados por patotipo (up y down)
  ...
  # 4. Tabla resumen de la anotación
  ...
  # 5. Parámetros del análisis (reproducibilidad) 
