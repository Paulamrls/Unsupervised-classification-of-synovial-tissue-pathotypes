...
# 1. Silhouette score para un vector de clusters

compute_silhouette <- function(cl, expr) {
  
  # Eliminamos muestras sin asignación de cluster que no sea válida
  valid <- !is.na(cl) & cl > 0
...
compute_bootstrap_stability <- function(cl_ref, expr, k, method_fn,
  ...
  for (b in seq_len(boot_n)) {
    ...
    # Alineación de las muestras entre referencia y bootstrap
    common   <- names(cl_b)
...
# 3. Validación biológica mediante el solapamiento con los patotipos histológicos clásicos
...
compute_biological_overlap <- function(cl, meta, pathotype_col = "pathotype_clean") {
  ...
  # Histologia solo para interpretacion posterior al clustering. Las muestras
  # sin etiqueta valida permanecen en el analisis, pero no entran en esta tabla.
  valid <- !is.na(pt_common) & pt_common != "Ungraded" & pt_common != ""
  cl_common <- cl_common[valid]
  pt_common <- pt_common[valid]
  if (length(pt_common) == 0L)
    return(list(ari = NA, table_abs = NULL, table_pct = NULL))
...
run_validation <- function(expr, clusters, meta, k,
  ...
  # Orden para el método de silueta y mejorar así la visualización de los resultados
